...
public class ChatServiceImpl implements ChatService {
    ...
    /** Query Router 使用的最近 N 轮历史对话。 */
    private static final int ROUTER_HISTORY_ROUNDS = 5;
    ...
    public ChatSendResult send(ChatSendRequestDTO dto) {
        ...
        try {
            List<ConversationMessage> routeHistory = loadRecentConversationHistory(session.getId(), ROUTER_HISTORY_ROUNDS);
            RoutedQuery routed = queryRouterService.route(QueryRouteRequest.builder()
                    .sessionId(session.getId())
                    .userId(dto.getUserId())
                    .originalQuery(dto.getContent())
        ...
        List<ConversationMessage> history = toConversationMessages(loadHistoryMessages(session.getId()));
     ...
     * route() 调用前用户消息已入库，因此最新一条需要跳过。
     */
    private List<ConversationMessage> loadRecentConversationHistory(Long dbSessionId, int rounds) {
        int fetchSize = rounds * 2 + 1;
        List<ChatMessage> descList = chatMessageMapper.selectList(new LambdaQueryWrapper<ChatMessage>()
                .eq(ChatMessage::getSessionId, dbSessionId)
        ...
        List<ChatMessage> historyDesc = new ArrayList<>(descList.subList(1, descList.size()));
        Collections.reverse(historyDesc);
        return toConversationMessages(historyDesc);
    }

    private List<ConversationMessage> toConversationMessages(List<ChatMessage> messages) {
