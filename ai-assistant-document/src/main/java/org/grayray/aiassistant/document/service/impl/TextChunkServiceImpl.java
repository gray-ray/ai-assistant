...
public class TextChunkServiceImpl implements TextChunkService {
    ...
    public List<TextChunk> chunk(String cleanedText, Long documentId, String documentName) {
        return chunk(cleanedText, null, documentId, documentName);
    }

    @Override
    public List<TextChunk> chunk(String cleanedText, Long knowledgeId, Long documentId, String documentName) {
        return chunk(cleanedText, knowledgeId, documentId, documentName, 1);
    }

    @Override
    public List<TextChunk> chunk(String cleanedText, Long knowledgeId, Long documentId,
                                 String documentName, Integer chunkVersion) {
        if (cleanedText == null || cleanedText.isBlank()) {
            return List.of();
        }
        int version = chunkVersion == null || chunkVersion < 1 ? 1 : chunkVersion;
        ...
        for (Chapter chapter : chapters) {
            ...
            for (int i = 0; i < overlapped.size(); i++) {
                ...
                allChunks.add(TextChunk.builder()
                        .chunkId(String.format("doc_%d_v%d_chunk_%d", documentId, version, globalIndex))
